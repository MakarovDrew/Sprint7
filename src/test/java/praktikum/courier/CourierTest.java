...
public class CourierTest {
    ...
    public void duplicateCourierCreationFailed() {
        ...
        duplicateResponse.assertThat()
                .statusCode(409)
                .body("message", equalTo(check.message("Этот логин уже используется. Попробуйте другой.")));
    }
    ...
    public void missingFieldsInCourierCreationFailed() {
        var courier = new Courier(null, "password", "firstName");
        ValidatableResponse createResponse = client.createCourier(courier);
        createResponse.assertThat()
                .statusCode(400)
                .body("message", equalTo(check.message("Недостаточно данных для создания учетной записи")));
    }
